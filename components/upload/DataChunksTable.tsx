      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Řádky</th>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {chunks.map((chunk, index) => (
            <tr key={index}>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{chunk.chunk.length} řádků</td>
              <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <PrimaryButton
                  onClick={() => handleUploadChunk(index)}
